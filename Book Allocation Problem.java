...
public class Main {
	...
	public static boolean read(int[] pages,int m,int capacity)
	{
		int stNum = 1; //no. of student initially first student
		int pagesRead = 0; // initially 0 pages read
		int bookNum=0; // initially book 0

		while(bookNum < pages.length)
		{
			if(pages[bookNum] + pagesRead <= capacity) 
			{
				pagesRead = pagesRead + pages[bookNum];
				bookNum++;
			}
			else
			{
				stNum++; //increase the student number 
				pagesRead = 0; //new student read 0 pages initially
			}
			
			if(stNum > m) {
				return false; //if no of student is greater than given students then return false
			}
		}	
		return true;
	}
